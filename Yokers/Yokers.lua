--- STEAMODDED HEADER
--- MOD_NAME: Yomi GAMBLE
--- MOD_ID: YomiGamble
--- MOD_AUTHOR: [Leftie]
--- MOD_DESCRIPTION: Adds new cards based on the turn-based fighting game Your Only Move Is HUSTLE, by ivysly.
...
SMODS.Joker {
  ...
  loc_txt = {
    ...
    text = {
      ...
      "This Joker gains {X:mult,C:white}X0.1 {} Mult per",
      "{C:attention} consecutive same{} hand played",
  ...
  config = { extra = { xmult = 1, lasthand = "High Card"} },
  -- loc_vars gives your loc_text variables to work with, in the format of #n#, n being the variable in order.
  -- #1# is the first variable in vars, #2# the second, #3# the third, and so on.
  -- It's also where you'd add to the info_queue, which is where things like the negative tooltip are.
  loc_vars = function(self, info_queue, card)
    return { vars = { card.ability.extra.xmult, card.ability.extra.lasthand } }
  end,
  ...
  calculate = function(self, card, context)
    ...
    if context.before and not context.blueprint then
      if context.scoring_name == card.ability.extra.lasthand then
        card.ability.extra.xmult = card.ability.extra.xmult + 0.1
      else
        card.ability.extra.xmult = 1
        card.ability.extra.lasthand = context.scoring_name
      end
    end
  end
}
...
SMODS.Joker {
  ...
  loc_txt = {
    ...
    text = {
      ...
      "between first scoring {C:attention}Pair{}",
      "{C:attention}#3#{} additional times",
      "{C:inactive}(#1#, #2#)"
    }
  },
  config = { extra = { first_card = nil, second_card = nil, repetitions = 1 } },
  rarity = 2,
  atlas = 'YomiGamble',
  pos = { x = 1, y = 0 },
  cost = 8,
  loc_vars = function(self, info_queue, card)
    return { vars = { card.ability.extra.first_card, card.ability.extra.second_card, card.ability.extra.repetitions } }
  end,
  calculate = function(self, card, context)
    if context.cardarea == G.play and context.individual then
      print(context.other_card.base.value)
      if card.ability.extra.first_card ~= nil and card.ability.extra.second_card == nil and context.other_card.base.value == card.ability.extra.first_card then
        card.ability.extra.second_card = context.other_card.base.value
        print('second card assigned:', card.ability.extra.second_card)
      end
      if card.ability.extra.first_card == nil then
        card.ability.extra.first_card = context.other_card.base.value
        print('first card assigned:', card.ability.extra.first_card)
      end
    end
    ...
    if context.cardarea == G.jokers and context.after then
      card.ability.extra.first_card = nil
      card.ability.extra.second_card = nil
      print('reset')
    end
  end
}
